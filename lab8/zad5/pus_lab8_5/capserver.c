...
int main(int argc, char** argv) {
    ...
    cap_t privilege_on, privilege_off, privilege_dropped;

    cap_value_t cap_list[1]; /* lista opcji capabilities */
    ...
    privilege_dropped = cap_init();

    if(port_number < 1024)
    {
        privilege_off = cap_get_proc();

        if(privilege_off == NULL)
        {
        	perror("cap_get_proc()");
        	exit(EXIT_FAILURE);
        }
