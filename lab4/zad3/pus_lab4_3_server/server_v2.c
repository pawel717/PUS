...
int main(int argc, char** argv) {
    ...
    if (argc != 3) {
        fprintf(stderr, "Invocation: %s <PORT NUMBER> <wybor 0 vs 1>\n", argv[0]);
        exit(EXIT_FAILURE);
    }
    wybor = atoi(argv[2]);
    ...
    if( wybor != 0 && wybor != 1)
    {
        fprintf(stderr, "Wybierz 0 lub 1");
        exit(EXIT_FAILURE);
    }
    ...
    while(1)
    {
        ...
        printf("Dane: %s", buffer);

        stream = s_sndrcvinfo.sinfo_stream;

        if(wybor == 0)
	    {
            printf("Strumien nr: %d\n", stream);
